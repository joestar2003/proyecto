      ...
      <nav className="navbar px-3 px-lg-5 d-flex align-items-center justify-content-between" style={{padding: '0.5rem 0', minHeight: 64}}>
        ...
        <div className="d-flex align-items-center gap-2" style={{minWidth: 180, justifyContent: 'flex-end'}}>
          ...
          <div className="position-relative">
            <button className="btn p-1" style={{background: 'none', border: 'none', color: '#6c63ff'}} onClick={()=>setShowSearch(v=>!v)} aria-label="Buscar">
            ...
            {showSearch && (
              <input
                ...
                autoFocus
                style={{position: 'absolute', top: 36, right: 0, minWidth: 180, background: '#232532', color: '#6c63ff', borderRadius: 8, fontSize: 15, zIndex: 300, border: '1.5px solid #6c63ff', boxShadow: '0 4px 16px #0008'}}
              />
            )}
          </div>
          {/* Si hay sesión, mostrar saludo y cerrar sesión */}
          {session && session.user ? (
            <>
              <span style={{color: '#444', fontWeight: 600, fontSize: 17, marginRight: 12}}>
                Hola, {session.user.name || session.user.email?.split("@")[0] || "usuario"}!
              </span>
              <button
                className="btn btn-outline-primary fw-bold"
                style={{borderRadius: 12, fontSize: 16, color: '#6c63ff', border: '2px solid #6c63ff', background: 'transparent', padding: '6px 18px'}}
            ...
            <>
              <Link href="/login" className="nav-link d-none d-lg-flex align-items-center" style={{color: '#6c63ff', fontWeight: 500, fontSize: 16}}>Ingresar</Link>
              <Link href="/registro" className="nav-link d-none d-lg-flex align-items-center" style={{color: '#6c63ff', fontWeight: 500, fontSize: 16}}>Registro</Link>
            </>
          )}
          {/* Carrito */}
          <Link href="/carrito" className="nav-link d-flex align-items-center position-relative" style={{color: '#6c63ff', fontWeight: 600, fontSize: 16, marginLeft: 4}}>
            <i className="bi bi-cart" style={{ fontSize: 22 }}></i>
            <span className="d-none d-lg-inline" style={{ marginLeft: 6 }}>Carrito{total > 0 ? ` (${total})` : ''}</span>
            {total > 0 && (
              <span style={{position: 'absolute', top: -6, right: -16, background: '#6c63ff', color: '#fff', borderRadius: '50%', fontWeight: 700, fontSize: 12, minWidth: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #fff', zIndex: 2}}>{total}</span>
            )}
          </Link>
          {/* Menú hamburguesa móvil */}
          <button className="d-lg-none btn p-2 ms-2" style={{background: 'none', border: 'none', color: '#6c63ff'}} onClick={()=>setShowMenu(v=>!v)} aria-label="Menú">
